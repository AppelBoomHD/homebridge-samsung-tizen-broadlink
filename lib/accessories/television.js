...
let ReceiverService = require('../services/receiver');
...
module.exports = class Television {
  constructor(device, platform, homebridge) {
    ...
    this.platformAccessory = new Homebridge.platformAccessory(
      this.device.config.name,
      this.UUID,
      Homebridge.hap.Accessory.Categories.TELEVISION
    );
  ...
  createServices() {
    ...
    this.services.speaker = this.device.config.receiver
      ? new ReceiverService(this, Homebridge)
      : new SpeakerService(this, Homebridge);
  ...
  getServices() {
    return [
      ...Object.values(this.services).map(
        (type) => type.services || type.service
      ),
      ...Object.values(this.inputs).map(
        (type) => type.services || type.service
      ),
    ].flat();
  }
  ...
  addAccessory(accessory) {
    if (!accessory || !accessory.services || !accessory.services.main) {
      return;
    }

    (
      accessory.services.main.services || [accessory.services.main.service]
    ).forEach((service) => {
      if (
        this.platformAccessory.getServiceById(
          Homebridge.hap.Service.Switch,
          service.subtype
        )
      ) {
        return;
